...
class CatBreedIdentifier {
    ...
    displayResults(predictions) {
        try {
            ...
            // Show email capture popup after 5 seconds (Growth strategy)
            setTimeout(() => {
                this.showEmailCapturePopup(predictions[0]);
            }, 5000);
        } catch (error) {
...
`;
    ...
    // Show Email Capture Popup (Growth Strategy)
    showEmailCapturePopup(topPrediction) {
        // Check if user has already seen this popup (use localStorage)
        const hasSeenPopup = localStorage.getItem('catbreedai_email_popup_seen');
        if (hasSeenPopup) {
        ...
        const breed = topPrediction.class;
        ...
        modal.innerHTML = `
            <div class="bg-white rounded-2xl p-8 max-w-md mx-4 shadow-2xl relative animate-slideUp">
                <button id="closeEmailModal" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600 text-2xl">×</button>
                
                <div class="text-center mb-6">
                    <div class="text-6xl mb-4">🎉</div>
                    <h3 class="text-2xl font-bold text-gray-900 mb-2">Congrats! Your cat is ${confidence}% ${breed}!</h3>
                    <p class="text-gray-600">Love your ${breed}? Stay connected for updates!</p>
                </div>
                
                <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl p-6 mb-6 relative">
                    <!-- Coming Soon Badge -->
                    <div class="absolute top-2 right-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white px-3 py-1 rounded-full text-xs font-bold">
                        Coming Soon
                    </div>
                    
                    <h4 class="font-bold text-gray-900 mb-2 text-center">🚀 AI Cat Care Guide</h4>
                    <p class="text-sm text-gray-600 mb-4 text-center">
                        We're creating personalized ${breed} guides!<br/>
                        Join <strong class="text-purple-600">2,847+</strong> cat lovers on the waitlist.
                    </p>
                    <ul class="text-sm text-gray-700 space-y-2">
                        <li class="flex items-start space-x-2">
                            <span class="text-purple-600">✓</span>
                            <span>AI-powered ${breed} breed insights</span>
                        </li>
                        <li class="flex items-start space-x-2">
                            <span class="text-purple-600">✓</span>
                ...
                <form id="popupEmailForm" class="space-y-3">
                    <input type="email" 
                           id="popupEmailInput" 
                           placeholder="your.email@example.com"
                           required
                           class="w-full px-4 py-3 border-2 border-purple-300 rounded-xl focus:border-purple-600 focus:outline-none transition focus:ring-2 focus:ring-purple-200">
                    <button type="submit" 
                            class="w-full bg-gradient-to-r from-purple-600 to-purple-800 hover:from-purple-700 hover:to-purple-900 text-white font-bold py-3 px-6 rounded-xl transition duration-200 shadow-lg">
                ...
                <button id="skipEmailCapture" class="w-full text-gray-500 hover:text-gray-700 text-sm mt-3">
                    Maybe later
                </button>
            </div>
        `;
        ...
        document.getElementById('closeEmailModal').addEventListener('click', () => {
            document.body.removeChild(modal);
            localStorage.setItem('catbreedai_email_popup_seen', 'true');
        });
        
        // Skip button event
        document.getElementById('skipEmailCapture').addEventListener('click', () => {
            document.body.removeChild(modal);
            localStorage.setItem('catbreedai_email_popup_seen', 'true');
        });
        
        // Form submit event
        document.getElementById('popupEmailForm').addEventListener('submit', (e) => {
            e.preventDefault();
            ...
            modal.innerHTML = `
                <div class="bg-white rounded-2xl p-8 max-w-md mx-4 shadow-2xl text-center">
                    ...
                    <p class="text-gray-600 mb-6">
                        We'll notify you at <strong>${email}</strong> as soon as our AI Cat Care Guide for ${breed} cats is ready!
                    </p>
                    <div class="bg-purple-50 rounded-xl p-4 mb-6">
                        <p class="text-sm text-purple-800">
                            🎯 <strong>Expected launch:</strong> Q1 2025<br/>
                            📧 Watch your inbox for updates
                        </p>
                    </div>
                    <button onclick="this.closest('#emailCaptureModal').remove()" 
                            class="bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 px-8 rounded-xl transition">
                        Continue
                    </button>
                </div>
            `;
